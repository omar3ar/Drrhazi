...
const PathologiesTable: React.FC = () => {
  ...
  return (
    <section id="pathologies" className="py-24 bg-white relative">
      {/* Background elements */}
      <div className="absolute top-0 rtl:left-0 rtl:right-auto right-0 w-[800px] h-[800px] bg-secondary/5 rounded-full blur-3xl -translate-y-1/2 rtl:-translate-x-1/3 translate-x-1/3 pointer-events-none" />
              ...
              <button
                ...
                className={`flex items-center gap-4 p-5 rounded-2xl transition-all duration-300 rtl:text-right text-left ${
                  activeTab === category.id
                    ...
                    : 'bg-transparent text-slate hover:bg-gray-100 hover:text-primary-dark'
                }`}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors shrink-0 ${
                  activeTab === category.id ? 'bg-white/20 text-white' : 'bg-white text-primary shadow-sm'
                }`}>
                  {category.icon}
                </div>
                    ...
                    return (
                      <div key={`content-${category.id}`}>
                        <div className="flex items-center gap-4 mb-8 border-b border-gray-200 pb-6">
                          ...
                          <h4 className="text-2xl md:text-3xl font-bold text-primary-dark">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {items.map((item, index) => (
                            ...
                            >
                              <h5 className="font-bold text-primary-dark text-lg mb-2 flex items-center gap-2">
                        ...
                        <div className="mt-10 p-6 bg-secondary/10 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-6">
                          <p className="text-secondary-dark font-medium max-w-md">
                            {t('pathologies.cta_text')}
                          </p>
                          <a 
                            href={`https://wa.me/33781861316?text=${whatsappMessage}`}
                            target="_blank" rel="noopener noreferrer"
                            className="whitespace-nowrap px-6 py-3 bg-accent text-white rounded-full font-medium hover:bg-accent-hover transition-colors"
                          >
